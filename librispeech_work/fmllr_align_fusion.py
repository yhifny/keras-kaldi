...
aligner = MappedAligner.from_files(config.final_model, 
            ...
            beam=1000.0, transition_scale=1.0, 
            self_loop_scale=1.0, 
            acoustic_scale=1.0)
...
models_dir_path = "/media/lumi/alpha/asr_work_cnn_gru_seq/model.hdf5" 
models=[]

for  filename in glob.glob(models_dir_path):
    print (filename)
    kalid_model = load_model(filename, compile=False, custom_objects={'GatedConvBlock':GatedConvBlock,
                                                        'NovoGrad': NovoGrad, 
...
with SequentialMatrixReader(feats_rspecifier) as f, open(config.train_transcription_file) as t:
    for (fkey, feats), line in zip(f, t):
        try:
            ...
            print("Exception", fkey, flush=True)
