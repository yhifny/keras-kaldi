...
#from tensorflow.keras.engine.topology import Layer, Input
from tensorflow.keras.layers import Conv1D, ZeroPadding1D, Multiply
from tensorflow.keras.models import Model

import numpy as np

class GatedConv1D:
    """tensorflow.keras-style class implementation of a Gated CNN layer.
    ...
    # Arguments
        ouput_dim (int)
        kernel_size (int)
        kwargs_conv (dict) : dictionary of keyword args for tensorflow.keras Conv1D layer
        kwargs_gate (dict) : dictionary of keyword args for gated tensorflow.keras Conv1D layer
    """
...
class TestGConv(unittest.TestCase):
    def setUp(self):
        ...
        x = GatedConv1D(output_dim, kernel_size=kernel, kwargs_conv={'padding':'same'},
                    kwargs_gate={'padding':'same'})(inp) 
